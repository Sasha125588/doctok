...
**Date:** 2026-04-19
**Reference:** `apps/web/design/doctok_desktop_v6.html`
...
- Quiz, Courses, Saved, Search, Progress pages.
...
| `FocusMode.vue` | Single-card column: `<CardMeta>`, `<PostTitle>`, `<PostCardBody>` (reused), `<RelatedTags>`, and a right-aligned `<ActionsColumn>`. Wraps card in `motion.div` with fade+slide on `activePostIndex`/`activeTopicSlug` change (AnimatePresence mode="wait"). |
...
| `ActionsColumn.vue` | Vertical column of icon buttons: like, save, note, comment, share. Counts under icons where applicable (`likeCount`, `commentCount`). Spring scale pop on toggle. |
| `RelatedTags.vue` | Row of tag buttons. Data from `useTopicLinks({slug})`. Click navigates to that topic if it resolves (via client-side router push). |
...
### Topics list
- `useFeed(lang)` (existing) → infinite list of topics.
...
### Active topic
- `activeTopicSlug` in `useFeedView`. Defaulted to first topic from `useFeed` on first render.
- When it changes: push to `useTopicHistory.addRecent(slug)`, reset `activePostIndex = 0`.

### Posts for active topic
- `useTopicPosts({slug: activeTopicSlug, lang})` (existing) → posts.
- `activePostIndex` selects current post.
- `progress dots` width = total posts count.

### Related tags
- `useTopicLinks({slug: activeTopicSlug, lang})` (existing) → `TopicLink[]`.
...
- `ArrowUp` → previous topic in feed list order
- `ArrowDown` → next topic in feed list order

### Like / vote
- Uses existing `useVote`. No change.
...
### Share
- `navigator.share({url})` with fallback to copy-to-clipboard + toast. URL = `/topic/${topicSlug}` with post anchor (post IDs exposed as `id`, so `#post-${id}`).
